...
import { View, Text, TextInput, TouchableOpacity, FlatList, KeyboardAvoidingView, Platform, ActivityIndicator, Image, Alert, Modal, Keyboard } from 'react-native';
...
export const options = ({ params }) => ({
  title: `Chat with User ${params.id}`,
});
...
export default function UserChatScreen() {
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      ...
      try {
        ...
        console.log('Message IDs:', res.data.messages?.map(m => ({ id: m.id, created_at: m.created_at })));
  ...
  const getUserAvatarInitials = (userName: string) => {
    return userName
      .split(' ')
  ...
  return (
    ...
    >
      {/* Header */}
      <View className={`flex-row items-center p-4 border-b ${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
